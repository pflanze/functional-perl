...
sub xlinkreplace {
    ...
    my ($source,$dest)=@_;
    ## schon wieder dieser huere temporary try mechanismus. sollte ich dringend eine generische func oder ein makro dafür haben theoretisch
...
{
    package Chj::xperlfunc::xstat; ## Alternative zu arrays: hashes, so dass slices à la ->{"dev","ino"} gemacht werden können. Schade kann man nicht alles haben.
    sub dev     { shift->[0] }
    sub ino     { shift->[1] }
    sub mode    { shift->[2] }
    sub nlink   { shift->[3] }
    ...
    sub setuid { !!(shift->[2] & 04000) } # keine lust, is_ vornedranzuhängen. Es ist ein boolean, wenn man das in perl nicht angeben kann schade
...
sub xmvmkdir {
    ...
    my ($owner,$group,$mode)= (xstat $_[1])[4,5,2];# Can't call method "xstat" on an undefined value at /root/extlib/Chj/xperlfunc.pm line 121. ????????  ah das war bevor ich die sub definition hinauf schob. Dann rät er also falsch   hmmmmmm. huree indirect object notation kack macht sogar dass infixehoutfix notation nich mehr rech geht.
    #my ($owner,$group,$mode)= (xstat ($_[1]))[4,5,2];
...
sub xutime {
    ...
    #utime @_
    #  or croak "xutime @_: $!";  # auch hier, wenn ich selber loope statt viele files an perl geben könnt ich melden bei welchem file es croakt
...
__END__
  ...
  HMMM, probleme:
- chdir wohin?
  - neuöffnen von stdin stdout stderr wohin? null? log?
  na, eher: sub xlauncher (&;..) ?  Wo ein callback dazwischen ausgeführt wird?
sub xlaunch {
...
but xmvmkdir seems schlauer à la mvcp
