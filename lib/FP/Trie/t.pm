...
TEST { [$t->perhaps_ref(string_to_list("Hell"))] }
[];

TEST { [$t->perhaps_ref(string_to_list("Hello"))] }
["World"];
...
my $t2 = $t->update(string_to_list("Hell"), sub { [@_, "yes"] });
my $t3 = $t2->update(string_to_list("Hello"), sub { [@_, "2"] });
...
TEST { [$t5->perhaps_ref(string_to_list("Hell"))] }
[];

TEST { [$t5->perhaps_ref(string_to_list("Hello"))] }
[["World", "2"]];

my $t6 = $t4->delete(string_to_list("Hello"));

TEST { [$t6->perhaps_ref(string_to_list("Hell"))] }
["no"];

TEST { [$t6->perhaps_ref(string_to_list("Hello"))] }
[];
...
sub random_key {
    my $len = int rand 3;
    join("", map { chr(32 + int rand 64) } 1 .. $len)
}
...
TEST {
    ...
    withrepl {
        ...
        for my $i (1 .. 1000) {
            my $k = random_key;
            &$set_both($k, $i);
            $trie->ref(string_to_list $oldk) == $$hash{$oldk} or die "bug";
            $oldk = $k;
        DEL: {
                my $k2 = random_key;
...
my $t7 = $t4->set(string_to_list("Hare"), "Krishna")
    ->set(string_to_list("He"), "Helium")->set(string_to_list("H"), "Heroic");
