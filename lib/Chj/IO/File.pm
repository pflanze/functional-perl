...
sub xbinmode {
    my $self = shift;
    if (@_ == 1) {
        binmode($self->fh, $_[0]) or die "binmode: $!";
    } elsif (@_ == 0) {
        binmode($self->fh) or die "binmode: $!";
    } else {
...

{
    ...
    sub xreadline_backwards {
     ...
     #warn "debug xreadline_backwards: start of file reached and nothing to go";
            ...
            $self->xseek(
                -($curpos > $SLICE_LENGTH ? $SLICE_LENGTH : $len_to_go),
                SEEK_CUR);
            my $totbuf;
            while ($len_to_go) {
                my $buf;
                if (my $len = $self->xread($buf, $len_to_go)) {
                    ...
                    if ($len_to_go < 0) {
                        die
                            "strange error (bug?, or maybe file changed while reading?), len_to_go = $len_to_go";
                    }
                } else {
...
#return;
            ...
            if ($newpos > 0) {    # there is something left to be read from top
                ...
                while (length($$bufferp)
                    and $$bufferp
                    =~ s/^(\Q$REVERSELINEBREAK\E)(? = \Q$REVERSELINEBREAK\E)//s
                ...
                while (length($$bufferp)
                    and $$bufferp
                    =~ s/^(\Q$REVERSELINEBREAK\E)(\Q$LINEBREAK\E|\z)/$2/s
...
sub xsendfile_to {
    ...
    if (0 && $use_sendfile) {
        ...
        if ($!) {
            if ($! == EINVAL) {
                warn
                    "xsendfile_to: got EINVAL, trying non-sendfile version instead"
                    if $DEBUG;
...
sub xrewind {
    my $self = shift;
    seek $self->fh, 0, 0 or croak "xrewind on " . ($self->quotedname) . ": $!";
    sysseek $self->fh, 0, 0
...
sub xtell {
    my $self = shift;
    @_ == 0 or croak "xtell: wrong number of arguments";
    my $res = tell $self->fh;
    if ($res == -1) {
...
sub tell {
    my $self = shift;
    @_ == 0 or croak "tell: wrong number of arguments";
    my $res = tell $self->fh;
    if ($res == -1) {
        undef
    } else {
...
sub xdup {    # (return objects)
    my $self = shift;
    warn
        "xdup: this method is unfinished and only can create output filehandles yet";
    my $myfileno = CORE::fileno $self->fh;
    defined $myfileno
        or croak "xdup: filehandle of "
        . ($self->quotedname)
        . " is undefined (maybe it's closed?)";
    require POSIX;
...
sub xflush {
    my $self = shift;
    require IO::Handle;
    IO::Handle::flush($self->fh) or die "xflush " . $self->quotedname . ": $!";
}
...
sub xunlink {
    my $self = shift;
    my $path = $self->xpath;
    unlink $path or croak "xunlink '$path': $!";
    $self->unset_path;
}
...
sub xlink {
    ...
    my ($newpath) = @_;
    my $path      = $self->xpath;
    link $path, $newpath or croak "xlink '$path','$newpath': $!";
}
...
sub xrename {
    ...
    my ($to) = @_;
    my $path = $self->xpath;
    rename $path, $to or croak "xrename '$path' to '$to': $!";
    $self->set_path($to);
}
...
sub xlinkunlink {
    ...
    my ($newpath) = @_;
    my $path      = $self->xpath;
    link $path, $newpath or croak "xlinkunlink: link '$path','$newpath': $!";
    unlink $path or croak "xlinkunlink: unlink '$path': $!";
